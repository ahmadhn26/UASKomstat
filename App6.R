  ...
  output$download_descriptive_interpretation_pdf <- downloadHandler(
    ...
    content = function(file) {
      ...
      tryCatch({
        ...
        rmd_content <- paste0(
          "---\n",
          "title: 'Laporan Lengkap Statistik Deskriptif - SEVA'\n",
          "author: 'Socio-Economic Vulnerability Analyzer'\n",
          "date: '", Sys.Date(), "'\n",
          "output:\n",
          "  word_document:\n",
          "    toc: true\n",
          ...
          "# Load the data\n",
          "sovi_data <- read.csv('data/sovi_data.csv')\n",
          "desc_data <- sovi_data[, c(", paste(paste0("'", input$desc_vars, "'"), collapse = ", "), ")]\n",
          "```\n\n",
          
          "# Ringkasan Eksekutif\n\n",
          "Laporan ini menyajikan analisis statistik deskriptif komprehensif untuk ", length(input$desc_vars), " variabel yang dipilih dalam sistem SEVA. Analisis mencakup statistik deskriptif lengkap, visualisasi distribusi, analisis outliers, dan interpretasi mendalam untuk mendukung pengambilan keputusan dalam konteks kerentanan sosial-ekonomi.\n\n",
          
          "## Variabel yang Dianalisis\n\n",
          "Variabel yang dipilih untuk analisis: **", paste(input$desc_vars, collapse = ", "), "**\n\n",
          
          "# Analisis Statistik Deskriptif\n\n",
          "## Tabel Statistik Deskriptif Lengkap\n\n",
          "```{r descriptive-table}\n",
          "# Calculate comprehensive descriptive statistics\n",
          "calc_desc_stats <- function(x) {\n",
          "  x <- x[!is.na(x)]\n",
          ...
          "    Mode = round(as.numeric(names(sort(table(x), decreasing = TRUE))[1]), 4),\n",
          "    SD = round(sd(x), 4),\n",
          ...
          "desc_summary <- data.frame(t(sapply(desc_data, calc_desc_stats)))\n",
          "desc_summary$Variable <- rownames(desc_summary)\n",
          "desc_summary <- desc_summary[, c('Variable', 'N', 'Mean', 'Median', 'Mode', 'SD', 'Variance', 'Min', 'Max', 'Range', 'Skewness', 'Kurtosis')]\n",
          "rownames(desc_summary) <- NULL\n\n",
          
          "kable(desc_summary, caption = 'Statistik Deskriptif Lengkap', digits = 4)\n",
          "```\n\n",
          
          "## Visualisasi Distribusi Data\n\n",
          "### Histogram untuk Setiap Variabel\n\n",
          "```{r histograms, fig.width=12, fig.height=", ceiling(length(input$desc_vars)/2) * 4, "}\n",
          "library(gridExtra)\n",
          "plots <- list()\n",
          "for(i in 1:ncol(desc_data)) {\n",
          "  var_name <- names(desc_data)[i]\n",
          "  plots[[i]] <- ggplot(desc_data, aes_string(x = var_name)) +\n",
          "    geom_histogram(bins = 30, fill = 'steelblue', alpha = 0.7, color = 'white') +\n",
          "    geom_vline(aes(xintercept = mean(desc_data[[var_name]], na.rm = TRUE)), \n",
          "               color = 'red', linetype = 'dashed', size = 1) +\n",
          "    geom_vline(aes(xintercept = median(desc_data[[var_name]], na.rm = TRUE)), \n",
          "               color = 'orange', linetype = 'dashed', size = 1) +\n",
          "    labs(title = paste('Distribusi', var_name),\n",
          "         subtitle = paste('Mean (merah):', round(mean(desc_data[[var_name]], na.rm = TRUE), 3), \n",
          "                         '| Median (orange):', round(median(desc_data[[var_name]], na.rm = TRUE), 3)),\n",
          ...
          "```{r boxplots, fig.width=12, fig.height=", ceiling(length(input$desc_vars)/2) * 4, "}\n",
          "box_plots <- list()\n",
          "for(i in 1:ncol(desc_data)) {\n",
          "  var_name <- names(desc_data)[i]\n",
          "  box_plots[[i]] <- ggplot(desc_data, aes_string(y = var_name)) +\n",
          "    geom_boxplot(fill = 'lightblue', alpha = 0.7, outlier.color = 'red', outlier.size = 2) +\n",
          "    stat_summary(fun = mean, geom = 'point', shape = 18, size = 3, color = 'red') +\n",
          "    labs(title = paste('Box Plot', var_name),\n",
          "         subtitle = paste('Red diamond: Mean | Red dots: Outliers'),\n",
          "         y = var_name) +\n",
          "    theme_minimal() +\n",
          "    theme(plot.title = element_text(size = 12, face = 'bold'),\n",
          "          plot.subtitle = element_text(size = 10))\n",
          ...
          "```{r central-tendency-analysis, results='asis'}\n",
          "for(var in names(desc_data)) {\n",
          "  var_data <- desc_data[[var]]\n",
          "  var_data <- var_data[!is.na(var_data)]\n",
          "  \n",
          ...
          "  cat('- **Perbedaan:** ', round(diff_pct, 2), '%\\n\\n')\n",
          "  \n",
          "  if(diff_pct < 5) {\n",
          "    cat('**Interpretasi:** Data memiliki distribusi yang relatif simetris dengan mean dan median yang hampir sama.\\n\\n')\n",
          "  } else if(mean_val > median_val) {\n",
          "    cat('**Interpretasi:** Data menunjukkan positive skew (ekor panjang ke kanan), dengan beberapa nilai ekstrem tinggi yang menarik mean ke atas.\\n\\n')\n",
          "  } else {\n",
          "    cat('**Interpretasi:** Data menunjukkan negative skew (ekor panjang ke kiri), dengan beberapa nilai ekstrem rendah yang menarik mean ke bawah.\\n\\n')\n",
          "  }\n",
          "}\n",
          "```\n\n",
          
          "## Analisis Variabilitas dan Dispersi\n\n",
          "```{r variability-analysis, results='asis'}\n",
          "cv_data <- data.frame(\n",
          "  Variable = names(desc_data),\n",
          "  CV = sapply(desc_data, function(x) {\n",
          "    x <- x[!is.na(x)]\n",
          "    round((sd(x) / mean(x)) * 100, 2)\n",
          "  })\n",
          ")\n",
          "\n",
          "kable(cv_data, caption = 'Coefficient of Variation (%)')\n",
          "\n",
          "cat('\\n\\n**Interpretasi Coefficient of Variation:**\\n\\n')\n",
          "for(i in 1:nrow(cv_data)) {\n",
          "  var_name <- cv_data$Variable[i]\n",
          "  cv_val <- cv_data$CV[i]\n",
          "  \n",
          "  cat('- **', var_name, ':** ', cv_val, '%')\n",
          "  if(cv_val < 15) {\n",
          "    cat(' (Variabilitas rendah - data homogen)\\n')\n",
          "  } else if(cv_val < 30) {\n",
          "    cat(' (Variabilitas sedang)\\n')\n",
          "  } else {\n",
          "    cat(' (Variabilitas tinggi - data heterogen)\\n')\n",
          "  }\n",
          "}\n",
          "```\n\n",
          
          "## Analisis Outliers\n\n",
          "```{r outlier-analysis, results='asis'}\n",
          "outlier_summary <- data.frame(\n",
          "  Variable = character(),\n",
          "  Outliers_Count = numeric(),\n",
          "  Outliers_Percentage = numeric(),\n",
          "  stringsAsFactors = FALSE\n",
          ")\n\n",
          ...
          "    Outliers_Percentage = round((length(outliers) / length(var_data)) * 100, 2)\n",
          "  ))\n",
          "}\n\n",
          
          "kable(outlier_summary, caption = 'Analisis Outliers')\n",
          "```\n\n"
        )
        
        # Add correlation analysis if multiple variables
        if(length(input$desc_vars) > 1) {
          rmd_content <- paste0(rmd_content,
            "## Analisis Korelasi Antar Variabel\n\n",
            "```{r correlation-analysis, fig.width=10, fig.height=8}\n",
            "# Calculate correlation matrix\n",
            "cor_matrix <- cor(desc_data, use = 'complete.obs')\n",
            "\n",
            ...
            "kable(round(cor_matrix, 3), caption = 'Matriks Korelasi Numerik')\n",
            "```\n\n"
          )
        }
        ...
        rmd_content <- paste0(rmd_content,
          ...
          "cat('Berdasarkan analisis statistik deskriptif yang telah dilakukan, ditemukan karakteristik sebagai berikut:\\n\\n')\n",
          "\n",
          ...
          "  skew_val <- moments::skewness(var_data)\n",
          "  \n",
          "  cat('### ', var, '\\n')\n",
          "  cat('- Rata-rata: ', round(mean_val, 3), '\\n')\n",
          "  cat('- Tingkat variabilitas: ', \n",
          "      ifelse(cv_val < 15, 'Rendah', ifelse(cv_val < 30, 'Sedang', 'Tinggi')), \n",
          "      ' (CV = ', round(cv_val, 1), '%)\\n')\n",
          "  cat('- Bentuk distribusi: ', \n",
          "      ifelse(abs(skew_val) < 0.5, 'Simetris', \n",
          "             ifelse(skew_val > 0, 'Positive Skew', 'Negative Skew')), '\\n\\n')\n",
          "}\n",
          "```\n\n",
          
          "## Rekomendasi untuk Analisis Lanjutan\n\n",
          "1. **Uji Normalitas:** Lakukan uji normalitas formal (Shapiro-Wilk, Anderson-Darling) untuk menentukan metode analisis yang tepat\n\n",
          "2. **Transformasi Data:** Untuk variabel dengan skewness tinggi, pertimbangkan transformasi logaritmic atau square root\n\n",
          "3. **Treatment Outliers:** Evaluasi outliers yang teridentifikasi - apakah perlu dipertahankan, ditransformasi, atau dihapus\n\n",
          "4. **Analisis Inferensia:** Gunakan hasil karakteristik distribusi untuk memilih uji statistik yang sesuai\n\n",
          "5. **Modeling:** Variabel dengan korelasi tinggi dapat dipertimbangkan untuk analisis regresi atau clustering\n\n",
          "## Metodologi\n\n",
          "Analisis ini menggunakan metode statistik deskriptif standar dengan bantuan R dan paket statistik. Semua perhitungan mengikuti prosedur standar dalam analisis eksploratori data (EDA) dan telah divalidasi untuk akurasi.\n\n",
          "---\n\n",
          "*Laporan ini dihasilkan secara otomatis oleh SEVA - Socio-Economic Vulnerability Analyzer pada ", Sys.Date(), "*"
        )
        
        writeLines(rmd_content, temp_rmd)
        rmarkdown::render(temp_rmd, output_file = file, quiet = TRUE, clean = TRUE)
      }, error = function(e) {
        stop("Failed to generate Word document: ", e$message)
      })
    },
  ...
  output$download_normality_test_pdf <- downloadHandler(
    ...
    content = function(file) {
      ...
      tryCatch({
        ...
        # Create comprehensive R Markdown content with embedded visualizations
        rmd_content <- paste0(
          "---\n",
          "title: 'Laporan Lengkap Uji Asumsi Statistik - SEVA'\n",
          "author: 'Socio-Economic Vulnerability Analyzer'\n",
          "date: '", Sys.Date(), "'\n",
          "output:\n",
          "  word_document:\n",
          ...
          "# Ringkasan Eksekutif\n\n",
          "Laporan ini menyajikan analisis komprehensif uji asumsi statistik untuk variabel **", input$normality_var, "** dalam sistem SEVA. Uji asumsi yang dilakukan mencakup uji normalitas menggunakan multiple methods, visualisasi distribusi, Q-Q plots, dan uji homogenitas varians (jika applicable). Hasil analisis ini penting untuk menentukan metode statistik yang tepat untuk analisis inferensia selanjutnya.\n\n",
          
          "## Variabel yang Dianalisis\n\n",
          "**Variabel:** ", input$normality_var, "\n\n",
          "**Ukuran Sampel:** ", length(var_data), " observasi (setelah penghapusan missing values)\n\n",
          
          "# Analisis Statistik Deskriptif\n\n",
          "```{r descriptive-stats}\n",
          "# Calculate comprehensive descriptive statistics\n",
          "desc_stats <- data.frame(\n",
          "  Statistik = c('Mean', 'Median', 'Mode', 'Standard Deviation', 'Variance', 'Minimum', 'Maximum', 'Range', 'Skewness', 'Kurtosis', 'Coefficient of Variation'),\n",
          "  Nilai = c(\n",
          "    round(mean(var_data), 4),\n",
          "    round(median(var_data), 4),\n",
          ...
          "kable(desc_stats, caption = paste('Statistik Deskriptif -', '", input$normality_var, "'))\n",
          "```\n\n",
          
          "```{r descriptive-interpretation, results='asis'}\n",
          "skew_val <- moments::skewness(var_data)\n",
          "kurt_val <- moments::kurtosis(var_data)\n",
          "cv_val <- (sd(var_data)/mean(var_data)) * 100\n\n",
          
          "cat('## Interpretasi Statistik Deskriptif\\n\\n')\n",
          "cat('### Tendensi Sentral\\n')\n",
          "cat('- **Mean vs Median:** ', round(mean(var_data), 4), ' vs ', round(median(var_data), 4))\n",
          "if(abs(mean(var_data) - median(var_data)) / median(var_data) * 100 < 5) {\n",
          "  cat(' (Distribusi relatif simetris)\\n')\n",
          "} else if(mean(var_data) > median(var_data)) {\n",
          "  cat(' (Indikasi positive skew)\\n')\n",
          "} else {\n",
          "  cat(' (Indikasi negative skew)\\n')\n",
          "}\n\n",
          
          "cat('### Variabilitas\\n')\n",
          "cat('- **Coefficient of Variation:** ', round(cv_val, 2), '%')\n",
          "if(cv_val < 15) {\n",
          "  cat(' (Variabilitas rendah - data homogen)\\n')\n",
          "} else if(cv_val < 30) {\n",
          "  cat(' (Variabilitas sedang)\\n')\n",
          ...
          "if(abs(skew_val) < 0.5) {\n",
          "  cat(' (Distribusi simetris)\\n')\n",
          "} else if(skew_val > 0.5) {\n",
          "  cat(' (Positive skew - ekor panjang ke kanan)\\n')\n",
          "} else {\n",
          "  cat(' (Negative skew - ekor panjang ke kiri)\\n')\n",
          "}\n\n",
          
          "cat('- **Kurtosis:** ', round(kurt_val, 4))\n",
          "if(kurt_val > 3) {\n",
          "  cat(' (Leptokurtik - lebih runcing dari normal)\\n')\n",
          "} else if(kurt_val < 3) {\n",
          "  cat(' (Platykurtik - lebih datar dari normal)\\n')\n",
          "} else {\n",
          "  cat(' (Mesokurtik - mendekati distribusi normal)\\n')\n",
          "}\n",
          "```\n\n",
          
          "# Visualisasi Distribusi Data\n\n",
          "## Histogram dan Density Plot\n\n",
          "```{r histogram-density, fig.width=12, fig.height=8}\n",
          ...
          "  geom_vline(xintercept = median(var_data), color = 'orange', linetype = 'dashed', size = 1) +\n",
          "  labs(title = paste('Distribusi', '", input$normality_var, "'),\n",
          "       subtitle = 'Histogram dengan Density Plot (merah) dan Kurva Normal Teoritis (biru putus-putus)',\n",
          "       x = '", input$normality_var, "', y = 'Density') +\n",
          "  theme_minimal() +\n",
          ...
          "  labs(title = 'Box Plot dengan Outliers',\n",
          "       subtitle = 'Red diamond: Mean | Red dots: Outliers',\n",
          "       y = '", input$normality_var, "') +\n",
          "  theme_minimal() +\n",
          "  theme(plot.title = element_text(size = 14, face = 'bold'))\n\n",
          ...
          "```{r qq-plots, fig.width=12, fig.height=8}\n",
          "# Create Q-Q plot\n",
          "p3 <- ggplot(data.frame(sample = var_data), aes(sample = sample)) +\n",
          "  stat_qq(color = 'steelblue', size = 2, alpha = 0.6) +\n",
          "  stat_qq_line(color = 'red', size = 1.2) +\n",
          "  labs(title = 'Q-Q Plot vs Normal Distribution',\n",
          "       subtitle = 'Points should follow red line for normal distribution',\n",
          "       x = 'Theoretical Quantiles', y = 'Sample Quantiles') +\n",
          "  theme_minimal() +\n",
          "  theme(plot.title = element_text(size = 14, face = 'bold'))\n\n",
          
          "# Create residuals plot if data resembles normal\n",
          "fitted_values <- seq(min(var_data), max(var_data), length.out = length(var_data))\n",
          "residuals_norm <- var_data - mean(var_data)\n",
          "p4 <- ggplot(data.frame(fitted = fitted_values, residuals = residuals_norm), aes(x = fitted, y = residuals)) +\n",
          "  geom_point(color = 'steelblue', alpha = 0.6) +\n",
          "  geom_hline(yintercept = 0, color = 'red', linetype = 'dashed') +\n",
          "  geom_smooth(method = 'loess', color = 'orange', se = TRUE) +\n",
          "  labs(title = 'Residuals vs Fitted Values',\n",
          "       subtitle = 'Should show random pattern around zero line',\n",
          ...
          "shapiro_test <- shapiro.test(var_data)\n",
          "ks_test <- ks.test(var_data, 'pnorm', mean(var_data), sd(var_data))\n",
          "ad_test <- nortest::ad.test(var_data)\n",
          "jb_test <- tseries::jarque.bera.test(var_data)\n\n",
          
          "# Create results table\n",
          "test_results <- data.frame(\n",
          "  Test = c('Shapiro-Wilk', 'Kolmogorov-Smirnov', 'Anderson-Darling', 'Jarque-Bera'),\n",
          "  Statistic = c(\n",
          "    round(shapiro_test$statistic, 6),\n",
          "    round(ks_test$statistic, 6),\n",
          "    round(ad_test$statistic, 6),\n",
          "    round(jb_test$statistic, 6)\n",
          "  ),\n",
          "  P_Value = c(\n",
          "    round(shapiro_test$p.value, 6),\n",
          "    round(ks_test$p.value, 6),\n",
          "    round(ad_test$p.value, 6),\n",
          "    round(jb_test$p.value, 6)\n",
          "  ),\n",
          "  Decision = c(\n",
          "    ifelse(shapiro_test$p.value > 0.05, 'Fail to reject H0 (Normal)', 'Reject H0 (Not Normal)'),\n",
          "    ifelse(ks_test$p.value > 0.05, 'Fail to reject H0 (Normal)', 'Reject H0 (Not Normal)'),\n",
          "    ifelse(ad_test$p.value > 0.05, 'Fail to reject H0 (Normal)', 'Reject H0 (Not Normal)'),\n",
          "    ifelse(jb_test$p.value > 0.05, 'Fail to reject H0 (Normal)', 'Reject H0 (Not Normal)')\n",
          "  )\n",
          ")\n\n",
          
          "kable(test_results, caption = 'Hasil Multiple Uji Normalitas')\n",
          "```\n\n",
          
          "### Hipotesis untuk Semua Uji Normalitas\n",
          "- **H₀:** Data berdistribusi normal\n",
          "- **H₁:** Data tidak berdistribusi normal\n",
          "- **α:** 0.05 (tingkat signifikansi)\n\n",
          
          "```{r normality-interpretation, results='asis'}\n",
          "normal_count <- sum(c(shapiro_test$p.value, ks_test$p.value, ad_test$p.value, jb_test$p.value) > 0.05)\n",
          "total_tests <- 4\n\n",
          
          "cat('## Interpretasi Komprehensif Uji Normalitas\\n\\n')\n",
          "cat('### Ringkasan Hasil\\n')\n",
          "cat('- **Jumlah uji yang mendukung normalitas:** ', normal_count, ' dari ', total_tests, '\\n')\n",
          "cat('- **Konsensus:** ')\n",
          "if(normal_count >= 3) {\n",
          "  cat('Data **DAPAT DIANGGAP BERDISTRIBUSI NORMAL** (mayoritas uji mendukung)\\n')\n",
          "} else if(normal_count >= 2) {\n",
          "  cat('Data **BORDERLINE NORMAL** (hasil mixed - perlu pertimbangan tambahan)\\n')\n",
          "} else {\n",
          "  cat('Data **TIDAK BERDISTRIBUSI NORMAL** (mayoritas uji menolak normalitas)\\n')\n",
          "}\n\n",
          
          "cat('### Analisis Per Uji\\n\\n')\n",
          "cat('#### 1. Shapiro-Wilk Test\\n')\n",
          "cat('- **Hasil:** W = ', round(shapiro_test$statistic, 4), ', p = ', round(shapiro_test$p.value, 4), '\\n')\n",
          "cat('- **Interpretasi:** ', ifelse(shapiro_test$p.value > 0.05, 'Mendukung normalitas', 'Menolak normalitas'), '\\n')\n",
          "cat('- **Keterangan:** Uji yang paling powerful untuk sampel kecil-menengah (<5000)\\n\\n')\n\n",
          
          "cat('#### 2. Kolmogorov-Smirnov Test\\n')\n",
          "cat('- **Hasil:** D = ', round(ks_test$statistic, 4), ', p = ', round(ks_test$p.value, 4), '\\n')\n",
          "cat('- **Interpretasi:** ', ifelse(ks_test$p.value > 0.05, 'Mendukung normalitas', 'Menolak normalitas'), '\\n')\n",
          "cat('- **Keterangan:** Membandingkan CDF empiris dengan CDF normal teoritis\\n\\n')\n\n",
          
          "cat('#### 3. Anderson-Darling Test\\n')\n",
          "cat('- **Hasil:** A = ', round(ad_test$statistic, 4), ', p = ', round(ad_test$p.value, 4), '\\n')\n",
          "cat('- **Interpretasi:** ', ifelse(ad_test$p.value > 0.05, 'Mendukung normalitas', 'Menolak normalitas'), '\\n')\n",
          "cat('- **Keterangan:** Lebih sensitif pada tail distribusi dibanding KS test\\n\\n')\n\n",
          
          "cat('#### 4. Jarque-Bera Test\\n')\n",
          "cat('- **Hasil:** JB = ', round(jb_test$statistic, 4), ', p = ', round(jb_test$p.value, 4), '\\n')\n",
          "cat('- **Interpretasi:** ', ifelse(jb_test$p.value > 0.05, 'Mendukung normalitas', 'Menolak normalitas'), '\\n')\n",
          "cat('- **Keterangan:** Berdasarkan skewness dan kurtosis, baik untuk sampel besar\\n\\n')\n",
          "```\n\n"
        )
        
        # Add homogeneity test content if applicable
        if(input$group_var != "None") {
          rmd_content <- paste0(rmd_content,
            "# Uji Homogenitas Varians\n\n",
            "```{r homogeneity-tests}\n",
            "# Prepare data for homogeneity test\n",
            "group_data <- sovi_data[['", input$group_var, "']]\n",
            "if(is.numeric(group_data)) {\n",
            "  group_data <- cut(group_data, breaks = 3, labels = c('Low', 'Medium', 'High'))\n",
            "}\n",
            ...
            "levene_test <- car::leveneTest(var_data_hom, group_data_hom)\n",
            "bartlett_test <- bartlett.test(var_data_hom, group_data_hom)\n\n",
            
            "# Create summary table\n",
            "homogeneity_results <- data.frame(\n",
            "  Test = c('Levene Test', 'Bartlett Test'),\n",
            "  Statistic = c(\n",
            ...
            "    ifelse(bartlett_test$p.value > 0.05, 'Homogeneous', 'Heterogeneous')\n",
            "  )\n",
            ")\n\n",
            
            "kable(homogeneity_results, caption = 'Hasil Uji Homogenitas Varians')\n",
            "```\n\n",
            
            "**Variabel Kelompok:** ", input$group_var, "\n\n",
            "### Hipotesis Homogenitas\n",
            "- **H₀:** Varians antar kelompok homogen (σ₁² = σ₂² = ... = σₖ²)\n",
            "- **H₁:** Varians antar kelompok tidak homogen\n\n",
            
            "```{r homogeneity-visualization, fig.width=12, fig.height=6}\n",
            "# Create boxplot by groups\n",
            "df_hom <- data.frame(Value = var_data_hom, Group = group_data_hom)\n",
            "p5 <- ggplot(df_hom, aes(x = Group, y = Value, fill = Group)) +\n",
            "  geom_boxplot(alpha = 0.7) +\n",
            "  geom_jitter(width = 0.2, alpha = 0.5) +\n",
            "  stat_summary(fun = mean, geom = 'point', shape = 18, size = 3, color = 'red') +\n",
            "  labs(title = paste('Distribusi', '", input$normality_var, "', 'per Kelompok'),\n",
            "       x = '", input$group_var, "', y = '", input$normality_var, "') +\n",
            "  theme_minimal() +\n",
            "  theme(legend.position = 'none')\n\n",
            
            "# Create variance comparison\n",
            "group_stats <- aggregate(Value ~ Group, df_hom, function(x) c(mean = mean(x), var = var(x), sd = sd(x)))\n",
            "group_stats_df <- data.frame(\n",
            "  Group = group_stats$Group,\n",
            "  Mean = round(group_stats$Value[,1], 4),\n",
            "  Variance = round(group_stats$Value[,2], 4),\n",
            "  SD = round(group_stats$Value[,3], 4)\n",
            ")\n\n",
            
            "p6 <- ggplot(group_stats_df, aes(x = Group, y = Variance, fill = Group)) +\n",
            "  geom_bar(stat = 'identity', alpha = 0.7) +\n",
            "  geom_text(aes(label = Variance), vjust = -0.3) +\n",
            "  labs(title = 'Perbandingan Varians Antar Kelompok',\n",
            "       x = '", input$group_var, "', y = 'Variance') +\n",
            "  theme_minimal() +\n",
            "  theme(legend.position = 'none')\n\n",
            ...
            "```{r homogeneity-table}\n",
            "kable(group_stats_df, caption = 'Statistik Deskriptif per Kelompok')\n",
            "```\n\n"
          )
        }
        ...
        rmd_content <- paste0(rmd_content,
          ...
          "normal_count <- sum(c(shapiro_test$p.value, ks_test$p.value, ad_test$p.value, jb_test$p.value) > 0.05)\n",
          "is_normal <- normal_count >= 3\n\n",
          
          "cat('## Status Asumsi Statistik\\n\\n')\n",
          "cat('### Normalitas\\n')\n",
          "if(is_normal) {\n",
          "  cat('- ✅ **ASUMSI NORMALITAS TERPENUHI**\\n')\n",
          "  cat('- Mayoritas uji (', normal_count, '/4) mendukung distribusi normal\\n')\n",
          "  cat('- Data dapat digunakan untuk uji parametrik\\n\\n')\n",
          "} else {\n",
          "  cat('- ❌ **ASUMSI NORMALITAS TIDAK TERPENUHI**\\n')\n",
          "  cat('- Mayoritas uji (', 4-normal_count, '/4) menolak distribusi normal\\n')\n",
        ...
        if(input$group_var != "None") {
          rmd_content <- paste0(rmd_content,
            ...
            "  cat('- ✅ **ASUMSI HOMOGENITAS TERPENUHI**\\n')\n",
            "  cat('- Levene test: p = ', round(levene_test$`Pr(>F)`[1], 4), ' > 0.05\\n\\n')\n",
            "} else if(exists('levene_test')) {\n",
            "  cat('- ❌ **ASUMSI HOMOGENITAS TIDAK TERPENUHI**\\n')\n",
            "  cat('- Levene test: p = ', round(levene_test$`Pr(>F)`[1], 4), ' < 0.05\\n\\n')\n",
            "}\n\n"
          )
        }
        ...
        rmd_content <- paste0(rmd_content,
          ...
          "if(is_normal) {\n",
          "  cat('### Uji Statistik yang Disarankan\\n')\n",
          "  cat('Karena asumsi normalitas terpenuhi, dapat menggunakan uji parametrik:\\n\\n')\n",
          "  cat('1. **T-test** untuk perbandingan mean (1 atau 2 sampel)\\n')\n",
          "  cat('2. **ANOVA** untuk perbandingan multiple groups\\n')\n",
          "  cat('3. **Regresi Linear** untuk analisis hubungan\\n')\n",
          "  cat('4. **Korelasi Pearson** untuk analisis korelasi\\n\\n')\n",
          "} else {\n",
          "  cat('### Alternatif yang Disarankan\\n')\n",
          "  cat('Karena asumsi normalitas tidak terpenuhi, pertimbangkan:\\n\\n')\n",
          "  cat('#### A. Transformasi Data\\n')\n",
          "  if(moments::skewness(var_data) > 0) {\n",
          "    cat('- **Log transformation** (untuk positive skew)\\n')\n",
          "    cat('- **Square root transformation**\\n')\n",
          "  } else {\n",
          "    cat('- **Square transformation** (untuk negative skew)\\n')\n",
          "    cat('- **Reflection + log transformation**\\n')\n",
          "  }\n",
          "  cat('\\n#### B. Uji Non-Parametrik\\n')\n",
          "  cat('- **Mann-Whitney U test** (alternatif t-test)\\n')\n",
          "  cat('- **Kruskal-Wallis test** (alternatif ANOVA)\\n')\n",
          "  cat('- **Spearman correlation** (alternatif Pearson)\\n')\n",
          "  cat('- **Wilcoxon signed-rank test** (paired samples)\\n\\n')\n",
          "}\n\n",
          
          "cat('## Keterbatasan Analisis\\n\\n')\n",
          "cat('1. **Ukuran Sampel:** n = ', length(var_data), ' (', \n",
          "    ifelse(length(var_data) < 30, 'sampel kecil - hasil uji mungkin kurang robust', \n",
          "           ifelse(length(var_data) < 100, 'sampel menengah', 'sampel besar')), ')\\n')\n",
          "cat('2. **Missing Values:** ', sum(is.na(sovi_data[['", input$normality_var, "']])), ' observasi dihapus\\n')\n",
          "if(abs(moments::skewness(var_data)) > 1) {\n",
          "  cat('3. **Skewness Tinggi:** Data memiliki skewness ekstrem (|skew| > 1)\\n')\n",
          "}\n",
          "cat('4. **Context Dependency:** Interpretasi harus mempertimbangkan konteks domain spesifik\\n\\n')\n",
          "```\n\n",
          
          "## Metodologi dan Referensi\n\n",
          "### Uji yang Digunakan\n\n",
          "1. **Shapiro-Wilk Test** - Optimal untuk n < 5000, high power\n",
          "2. **Kolmogorov-Smirnov Test** - General purpose, compares CDFs\n",
          ...
          "- **Key packages:** ggplot2, car, nortest, moments, gridExtra\n",
          "- **Significance level:** α = 0.05\n\n",
          "---\n\n",
          "*Laporan ini dihasilkan secara otomatis oleh SEVA - Socio-Economic Vulnerability Analyzer pada ", Sys.Date(), ". Untuk pertanyaan metodologi atau interpretasi lebih lanjut, silakan konsultasikan dengan ahli statistik.*"
        )
